...
public class ServerAccountDAO {
	...
	/**
	 * Returns a number of {@link ch.uzh.csg.coinblesk.model.ServerAccount}s 
	 * 
	 * @param page
	 * @return List of ServerAcoounts
	 */
	public List<ch.uzh.csg.coinblesk.model.ServerAccount> getServerAccounts(int page){
		if (page < 0) {
			return null;
		}
		
		CriteriaBuilder cb = em.getCriteriaBuilder();
		CriteriaQuery<ch.uzh.csg.coinblesk.model.ServerAccount> cq = cb.createQuery(ch.uzh.csg.coinblesk.model.ServerAccount.class);
		Root<ServerAccount> root = cq.from(ServerAccount.class);
		cq.select(cb.construct(ch.uzh.csg.coinblesk.model.ServerAccount.class, root.get("id"),root.get("url"), root.get("payinAddress"),
		...
		List<ch.uzh.csg.coinblesk.model.ServerAccount> resultWithAliasedBean = em.createQuery(cq)
