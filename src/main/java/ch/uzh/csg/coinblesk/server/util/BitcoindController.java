...
public class BitcoindController {
	...
	public static String sendCoins(String address, BigDecimal amount)throws BitcoinException {
		...
		try {
			BITCOIN.unlockWallet(credentials.getBitcoindEncryptionKey(), 2);
		} catch (Exception e) {
		...
		keyPoolCheck();
		return BITCOIN.sendFrom(credentials.getBitcoindUsername(), address, amount.doubleValue());
	}
	...
	public static String getNewAddress() throws BitcoinException {
		keyPoolCheck();
		String newAddress = BITCOIN.getNewAddress(credentials.getBitcoindUsername());
		return newAddress;
	}
	...
	public static void backupWallet() {
		try {
			Date date = new Date();
			BITCOIN.backupWallet(ServerProperties.getProperty("backup.dir") + "_" + date.toString());
	...
	protected static BigDecimal getAccountBalance() throws BitcoinException  {
		BigDecimal accountBalance = new BigDecimal(Double.toString(BITCOIN.getBalance(credentials.getBitcoindUsername(), Config.MIN_CONFIRMATIONS_SMALL_TRANSACTIONS)));
